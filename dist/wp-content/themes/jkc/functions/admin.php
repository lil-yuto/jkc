	...
	if ($get_posttype == 'event_schedule') {
		switch ($column_name) {
			...
			case 'menu_order': // メニューオーダー
				...
				break;
			case 'postid': $stitle = $post_id; break; // 投稿ID
			case 'taxonomy-event_category': $stitle = get_post_meta($post_id, 'taxonomy-event_category', true); break; // カテゴリー
			case 'taxonomy-event_area': $stitle = get_post_meta($post_id, 'taxonomy-event_area', true); break; // 都道府県
			case 'acf_ev_place': $stitle = get_post_meta($post_id, 'acf_ev_place', true); break; // 会場
			case 'acf_ev_office': $stitle = get_post_meta($post_id, 'acf_ev_office', true); break; // 事務所
		}
